      ...
      async.eachSeries(responseJSON, function(item,eachCallback){
        async.waterfall([
          ...
          function(callback){
            ...
            if(item['Planning_Authority'] !=null && item['Borough_Reference'] != null &&item['Permission_Status'] != null && item['Permission_Date'] != null && item['Post_Code']
               != null && item['Proposed_Total_Residential_Units'] != null && item['Proposed_Total_Affordable_Percentage'] != null && item['lat']
               != null && item['lon'] != null && item['Proposed_Total_Affordable_Units'] != null && item['Cluster_ID'] != null && item['Housing_Density']!= null){
              Application.forge({
                Planning_Authority:item['Planning_Authority'],
                Borough_Ref:item['Borough_Reference'],
                Permission_Status:item['Permission_Status'],
                Permission_Date :item['Permission_Date'],
                Post_Code:item['Post_Code'],
                Total_Residential_Units: item['Proposed_Total_Residential_Units'],
                Proposed_Total_Aff_Per: item['Proposed_Total_Affordable_Percentage'],
                lat: item['lat'],
                lon: item['lon'],
                Total_Aff_Units: item['Proposed_Total_Affordable_Units'],
                Cluster_ID: item['Cluster_ID'],
                Housing_Density: item['Housing_Density'],
...
app.post('/loadSchools', function (req, res) {
      ...
      async.eachSeries(responseJSON, function(item,eachCallback){
        async.waterfall([
          function(callback){
            // add to applications database
            if(item['Name'] != null && item['Level'] != null && item['Age_Group'] != null && item['lat'] != null && item['lon'] != null){
              School.forge({
                Name: item['Name'],
                Type: item['Level'],
        ...
        function(err, results){
          console.log("Error: " + err);
          console.log("Waterfall results: " + JSON.stringify(results));
          eachCallback();
        });
      });
...
app.post('/loadGPs', function (req, res) {
      ...
      async.eachSeries(responseJSON, function(item,eachCallback){
        async.waterfall([
          function(callback){
            // add to applications database
            if(item['Ward_Name'] != null && item['GPs'] != null && item['lat'] != null && item['lon'] != null){
              GP.forge({
        ...
        async.waterfall([
          ...
          function(greenSpace, callback){
            if(greenSpace){
              ...
              .then(function(applications){
                  // pass to calculate distances
                  callback(null, greenSpace, applications.toJSON());
              })// pass greenspace and all applications to calculate distances

            } else{
                callback(null, false, false);
            }
          }, // end callback 2
          function(greenSpace, applications, callback){

              if(greenSpace){
                  console.log("Pre JSON Applications: " + applications);
                  console.log("Applications: " + JSON.stringify(applications));
                  console.log("GreenSpace ID: " + greenSpace['id']);
                  console.log("GreenSpace: " + JSON.stringify(greenSpace));
                  // for each destination, make call to distance

                  async.eachSeries(applications, function(application, eachCallback){
...
app.post('/loadSupermarkets', function (req, res) {
      ...
      async.eachSeries(responseJSON, function(item,eachCallback){
        async.waterfall([
          function(callback){
            // add to applications database
            if(item['Retailer'] != null && item['lat'] != null && item['lon'] != null){
              Supermarket.forge({
